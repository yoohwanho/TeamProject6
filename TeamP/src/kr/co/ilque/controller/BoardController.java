...
public class BoardController {
	...
	public String writeOk(@RequestParam("category") String category, @RequestParam("title") String title,
			@RequestParam("contents") String contents, @RequestParam("reward") int reward,
			...
			@RequestParam("sample6_address2") String address2, HttpSession ss) {
	...
	public String modifyOk(@ModelAttribute("dvdto") DetailViewDto dvdto,
		 ...
		 * time=#{time},phone=#{phone},reward=#{reward} WHERE boardNo=#{boardNo}
		 */
		BoardDto bdto = new BoardDto();
		bdto.setCategory(dvdto.getCategory());
	...
	public ModelAndView modifyBoard(@RequestParam(name = "boardNo") int boardNo) {
		...
		mav.addObject("sample6_address", loc[0]);
